...
Implements Stage 2 with a pool of classifiers and pruning strategy.

Usage:
    python ensembleTrain.py
    python ensembleTrain.py --train path/to/train.csv --test path/to/test.csv
    python ensembleTrain.py --stage1-model path/to/iso_forest.pkl
"""
...
# Pipeline config
BATCH_SIZE_OPTIONS = [25000] # After experiments we leave the best option from this batch sizes [10000, 15000, 20000, 25000, 30000]
...
EVAL_DIR = Path("evaluation") / "ensemble"


def parse_args():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description="Train Dynamic Ensemble")
    parser.add_argument(
        "--train", 
        type=str, 
        required=True,
...
class DynamicEnsemble:
    ...
    def _create_base_estimator(self):
        """Create a new base MLP estimator."""
    ...
    def update(self, X_batch, y_batch):
        """
        Update ensemble with new batch.
        Trains a new model and prunes worst performer if pool is full.
        """
        ...
        if len(self.pool) < self.pool_size:
            ...
            # Evaluate pool on current batch and remove worst
            accuracies = [
...
def main(train_path, test_path, stage1_model_path, batch_size=None, output_dir=None, models_dir=None):
    ...
    if batch_size is None:
        ...
        if all_results:
            ...
            output_file = eval_dir / "ensemble_evaluation.csv"
...
def _train_single(train_path, test_path, stage1_model_path, batch_size, output_dir=None, models_dir=None):
    ...
    print("=" * 60)
    print("DYNAMIC ENSEMBLE TRAINING (SEA)")
    print("=" * 60)
    ...
    # =========================================================================
    # PHASE 1: WARMUP (Initialize pool)
    # =========================================================================
    print("\n" + "=" * 60)
    print("PHASE 1: WARMUP (INITIALIZING ENSEMBLE POOL)")
    print("=" * 60)
    ...
    # Initialize ensemble
    ensemble = DynamicEnsemble(pool_size=POOL_SIZE)
    
    if len(X_warmup) > 0:
        # Split warmup into chunks to fill the pool
        chunk_size = max(1, len(X_warmup) // POOL_SIZE)
        
        for i in range(POOL_SIZE):
            start_idx = i * chunk_size
            end_idx = min((i + 1) * chunk_size, len(X_warmup))
            
            if start_idx >= len(X_warmup):
                break
                
            ensemble.update(
                X_warmup.iloc[start_idx:end_idx], 
                y_warmup.iloc[start_idx:end_idx]
            )
        
        print(f"Ensemble initialized with {len(ensemble.pool)} models.")
    else:
    ...
    for i in range(n_batches):
        ...
        if len(suspicious_X) > 0:
            ...
            s2_pred = ensemble.predict(suspicious_X)
            y_batch_pred[suspicious_mask] = s2_pred
            
            # Dynamic update
            suspicious_y_true = y_batch[suspicious_mask]
            ensemble.update(suspicious_X, suspicious_y_true)
    ...
    # Save ensemble
    model_file = model_dir / "dynamic_ensemble.pkl"
    joblib.dump(ensemble, model_file)
    print(f"Saved ensemble to {model_file}")
