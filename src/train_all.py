...
Usage:
    ...
    python src/train_all.py --train data/train.csv --test data/test.csv --model mlp
    python src/train_all.py --train data/train.csv --test data/test.csv --model ensemble
    
    # Train only Stage 2 (skip Isolation Forest training)
    python src/train_all.py --train data/train.csv --test data/test.csv --model mlp --skip-stage1
    python src/train_all.py --train data/train.csv --test data/test.csv --model mlp --stage1-model models/iso_forest_0.45.pkl
"""
...
def train_isolation_forest(train_path: str, test_path: str, dataset_type: str) -> Path:
    ...
    iso_main(train_path=str(benign_path), test_path=test_path, output_dir=str(output_dir))
    
    return Path("models") / f"iso_forest_{DEFAULT_ISO_CONTAMINATION}.pkl"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to training CSV (IsoForest will use <name>_benign.csv)"
    )
    parser.add_argument(
        "--test",
        type=str,
        required=True,
        help="Path to test CSV (with ANOMALY column)"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    # Extract dataset type from path
    dataset_type = get_dataset_type(args.train)
    ...
    if skip_stage1:
        ...
        if args.stage1_model:
            ...
            stage1_model = Path("models") / f"iso_forest_{DEFAULT_ISO_CONTAMINATION}.pkl"
        ...
        benign_path = get_benign_path(args.train)
        if not benign_path.exists():
            print(f"\nError: Benign dataset not found: {benign_path}")
            sys.exit(1)
        
        # Train Stage 1
        stage1_model = train_isolation_forest(args.train, args.test, dataset_type)
